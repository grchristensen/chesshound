...
pub struct MoveTree<M: Clone + Move + Eq + Hash, G: ListMoves<M>> {
    // TODO: No reason not to allow empty games.
    game_tree: HashMap<M, InternalTree<M, G>>,
}

struct InternalTree<M: Clone + Move + Eq + Hash, G: ListMoves<M>>(
    Vec<G>,
    HashMap<M, Box<InternalTree<M, G>>>,
);
...
impl<M: Clone + Move + Eq + Hash, G: ListMoves<M>> MoveTree<M, G> {
    ...
    pub fn new(games: Vec<G>) -> MoveTree<M, G> {
        let mut game_tree: HashMap<M, InternalTree<M, G>> = HashMap::new();

        for game in games {
            let mut moves_iter = game.list_moves();

            let first_move = moves_iter.next().expect("Empty game");
            ...
            let mut current_position = if let Some(first_game_tree) = first_game_tree {
                ...
                game_tree.insert(first_move.clone(), InternalTree(Vec::new(), HashMap::new()));

                game_tree.get_mut(&first_move).unwrap()
            };

            for move_ in moves_iter {
                if let None = current_position.1.get(&move_) {
                    // Have to construct hashmap for the first time.
                    current_position.1.insert(
                        move_.clone(),
                        Box::new(InternalTree(Vec::new(), HashMap::new())),
                    );
                }

                current_position = current_position.1.get_mut(&move_).unwrap();
            }

            current_position.0.push(game);
        }
...
impl<'a, M: Clone + Move + Eq + Hash, G: ListMoves<M>> MoveTreeView<'a, M, G> {
    ...
    pub fn with_next(self, chess_move: &M) -> MoveTreeView<'a, M, G> {
        if let Some(game_tree) = self.game_tree {
            if let Some(next_position) = game_tree.1.get(chess_move) {
                MoveTreeView::new(Some(next_position))
            } else {
                MoveTreeView::new(None)
            }
...
impl<'a, M: Clone + Move + Eq + Hash, G: ListMoves<M>> Iter<'a, M, G> {
    fn new(game_tree: &Option<&'a InternalTree<M, G>>) -> Iter<'a, M, G> {
...
struct InternalIter<'a, M: Clone + Move + Eq + Hash, G: ListMoves<M>> {
    ...
    node_stack: Vec<hash_map::Iter<'a, M, Box<InternalTree<M, G>>>>,
}

impl<'a, M: Clone + Move + Eq + Hash, G: ListMoves<M>> InternalIter<'a, M, G> {
    fn new(game_tree: &'a InternalTree<M, G>) -> InternalIter<'a, M, G> {
        let node_stack = vec![game_tree.1.iter()];

        InternalIter {
            vec_iter: game_tree.0.iter(),
        ...
        if let Some(game) = self.vec_iter.next() {
            ...
            if let Some(game_tree_iter) = self.node_stack.last_mut() {
                if let Some((_, next_tree)) = game_tree_iter.next() {
                    self.node_stack.push(next_tree.1.iter());
                    self.vec_iter = next_tree.0.iter();
                } else {
                    self.node_stack.pop();
                };
